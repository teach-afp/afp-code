...
module LiveInterpreter2025Part3 where
...
newtype Eval a = Eval { unEval :: Env -> Store -> IO (a, Store) }
...
instance Monad Eval where
  return   = pure
  m >>= k  = Eval \ env store -> do
    (v, store') <- unEval m env store
    unEval (k v) env store'

instance Applicative Eval where
  (<*>) = ap
  pure v = Eval \ _env store -> return (v, store)

instance Functor Eval where
  fmap  = liftM

ask :: Eval Env
ask = Eval \ env store -> return (env, store)

local :: (Env -> Env) -> Eval a -> Eval a
local f m = Eval \ env store -> unEval m (f env) store

liftIO :: IO a -> Eval a
liftIO m = Eval \ _env store -> do
  a <- m
  return (a, store)

modify :: (Store -> Store) -> Eval ()
modify f = Eval \ env store -> return ((), f store)

get :: Eval Store
get = Eval \ env store -> return (store, store)
...
runEval :: Eval a -> IO a
runEval m = do
  (v, _store) <- unEval m emptyEnv emptyStore
  return v
