...
module LiveInterpreter2025Part1 where
...
type Eval a = Env -> IO a

runEval :: Eval a -> IO a
runEval m = m emptyEnv
...
-- Local variables
eval (Var x)        env = return $ Map.findWithDefault err x env
  where err = error $ "unbound variable: " ++ x
eval (Let x e1 e2)  env = do
  v1 <- eval e1 env
  v2 <- eval e2 (Map.insert x v1 env)
  return v2
